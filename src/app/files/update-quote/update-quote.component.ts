 ...
 ngOnInit(): void {
    ...
    this.quoteService.getQuoteById(quoteId).subscribe(
      res=>{
        ...
        this.invoice.clientid = res.quote.client_id
      this.selectedClient =  this.getClient(this.invoice.clientid)
      this.invoice.products = res.items
      this.invoice.tvaObj.ht_price = res.quote.ht_price
      this.invoice.tvaObj.price_tva = res.quote.price_tva
    ...
    let quoteId = this.activatedRoute.snapshot.params.id
       ...
       this.quoteService.updateQuote(quoteId, this.invoice.products,this.invoice.tvaObj,config).subscribe(
         res=>{
          this.toastr.success("Quote is Updated!")
